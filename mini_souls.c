...
//to tell which positions cause text to appear upon interation
uint16_t interact_text[]={
    1289, 1290
};
/*char text[20][248]={
    "We"
...
void main(void) {
    ...
    set_sprite_data(0, 13u, sprite_data);
    set_sprite_tile(0,0);
    set_sprite_tile(1,1);
    move_sprite(0,84, 80);
    move_sprite(1,160,160);
    ...
    while(1) {
        if (can_move==1)  {
            ...
            //interact
            if (joypad() & J_SELECT){
                for (needle=0;needle<sizeof(*interact_text);needle++){
                if (floor(plx+4-4*plright,8)+map_mapWidth*floor(ply+4-4*pldown,8)+plright+map_mapWidth*pldown==interact_text[needle]) {
                    printf("%d", floor(plx+4-4*plright,8)+map_mapWidth*floor(ply+4-4*pldown,8)+plright+map_mapWidth*pldown);
                }
                }
            }
            //open meny
            if ((joypad() & J_START) && !(joypadPrevious & J_START)){
                ...
                printf("Little Dwarf\n\n\nWeapon\n\nArmor\n\nItems\n\nBest friend");
                set_sprite_data(0, 14u, sprite_data);
                set_sprite_tile(sprite_num+2,13);
            ...
            //for the menu movement
                ...
                if ((joypad() & J_UP) && !(joypadPrevious & J_UP)) scroll_sprite(sprite_num+1, 0, -16);
                else if ((joypad() & J_DOWN) && !(joypadPrevious & J_DOWN)) scroll_sprite(sprite_num+1, 0, 16);
                if ((joypad() & J_START) && !(joypadPrevious & J_START)){
                camera_x = plx-76;
                camera_y = ply-64;
                set_camera();
